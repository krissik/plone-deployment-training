...
DEPLOY_ENV=prod
DEPLOY_HOST=plone-deployment-training.example.com
DEPLOY_PORT=22
DEPLOY_USER=plone
...
You need either a Ubuntu or Debian based system for plone-deployment-training.example.com, enable SSH, and install a supported version of Python 3 on that system.
...
prod:
  hosts:
    plone-deployment-training.example.com:
      ansible_user: root
      host: plone-deployment-training
      hostname: plone-deployment-training.example.com
```
...
Deploy the stack defined in `devops/stacks/plone-deployment-training.example.com.yml` to the remote server with:
